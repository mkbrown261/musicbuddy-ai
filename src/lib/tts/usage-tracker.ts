...
export async function saveVoicePreferences(
  ...
  try {
    ...
    await db.prepare(
      ...
      userId,
      cid,
      prefs.preferredProvider ?? 'openai',
      prefs.openaiVoice ?? 'shimmer',
      prefs.openaiVoiceLabel ?? null,
      prefs.elevenlabsVoice ?? '21m00Tcm4TlvDq8ikWAM',
      prefs.elevenlabsVoiceName ?? null,
      prefs.pollyVoice ?? 'Joanna',
      prefs.speed ?? 0.92,
      prefs.defaultEmotion ?? 'friendly',
      prefs.singingMode != null ? (prefs.singingMode ? 1 : 0) : 0,
      prefs.voiceGender ?? 'female',
      // Phase 2
      prefs.voiceCharacter ?? 'luna',
      prefs.voiceStyle ?? 'default',
      prefs.stability ?? 0.35,
      prefs.styleBoost ?? 0.75,
      prefs.similarity ?? 0.60,
      prefs.groqPersonality != null ? (prefs.groqPersonality ? 1 : 0) : 1,
    ).run();
  } catch (e) {
